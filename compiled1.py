...
def transaction():
    ...
    query4="select account from customer"
    cur.execute(query4)
    data1=cur.fetchall()
    ...
    for row in data1:   
        ...
        query_check = "SELECT COUNT(*) FROM transaction WHERE accno = %s"
        cur.execute(query_check, (accno,))
    ...
    def fetchtransac(accno): 
        ...
        table_exists = cur.fetchall()
        if table_exists:
            cur.execute('select * from transaction_{accno}')
    ...
    #conn=mysql.connector.connect(host="localhost",user="root",password="1234",database="bank")
    #cur=conn.cursor()
    x="select Balance from transaction where accno={}".format(account)
    cur.execute(x)
    bal=cur.fetchone()
